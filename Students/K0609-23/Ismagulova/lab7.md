...
Требуется сделать вхход в под и попробовать API. Внутри выполняем следующее: просмотр списка подов, попытка удаления пода и попытка заглянуть в другой Namespace
